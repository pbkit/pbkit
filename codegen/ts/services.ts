  ...
  return [
    ...
    Object.entries(service.rpcs).map(([rpcName, rpc]) => {
      ...
      return [
        ...
        `      serializeJson: (value: ${rpc.reqType.type}) => JSON.stringify(${encodeRequestJson}(value)),\n`,
        `    },\n`,
        `    responseType: {\n`,
        `      serializeBinary: ${encodeResponseBinary},\n`,
        `      deserializeBinary: ${decodeResponseBinary},\n`,
        `      serializeJson: (value: ${rpc.resType.type}) => JSON.stringify(${encodeResponseJson}(value)),\n`,
        `    },\n`,
        "  },\n",
      ].join("");
    }).join(""),
